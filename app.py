...
# 第二行：[周期下拉] ◀ ▶ （◀ 和下拉框位置已对调）
r2c1, r2c2, r2c3 = st.columns([0.70, 0.15, 0.15])
with r2c1:
...
# 各粒度对应抓取多少根原生K线，保证聚合到大周期(如15d/20d/45d)时仍有足够K线数量
BTC_TIMEFRAME_BY_BASE = {
    'minute': ('1m', BASE_FETCH_LIMIT),   # ~34.7天，够用于分钟级聚合
    'hour':   ('1h', 6000),               # ~250天，够用于2h~16h聚合
    'day':    ('1d', 2000),               # ~5.5年，够用于1d~45d聚合
}
...
def fetch_btc_ohlcv(timeframe, total_limit):
    exchange = ccxt.okx()
    per_call = 300
    step_ms = _timeframe_seconds(timeframe) * 1000
    all_bars = []
    since = exchange.milliseconds() - total_limit * step_ms
...
def get_btc_base_df(base_type):
    timeframe, total_limit = BTC_TIMEFRAME_BY_BASE[base_type]
    bars = fetch_btc_ohlcv(timeframe, total_limit)
...
def resample_data(symbol, rule):
    ...
    df = get_gold_df(base_type) if symbol=="GC=F" else get_btc_base_df(base_type)
