...
with col_btc:
    if st.button("BTC/USDT", use_container_width=True,
                 type="primary" if st.session_state.selected_symbol == "BTC/USDT" else "secondary"):
        st.session_state.selected_symbol = "BTC/USDT"
with col_gold:
    if st.button("黄金 GOLD", use_container_width=True,
                 type="primary" if st.session_state.selected_symbol == "黄金 GOLD" else "secondary"):
        st.session_state.selected_symbol = "黄金 GOLD"
...
with c2:
    st.markdown(
        f"<div style='text-align:center;font-size:18px;font-weight:bold;padding:4px;color:white'>"
        f"{selected_symbol_label} · {st.session_state.selected_label}</div>",
...
const chart1 = LightweightCharts.createChart(document.getElementById('chart1'), {{
  width: w, height: Math.floor(h * 0.62),
...
// EMA24 橙色
const ema24Series = chart1.addLineSeries({{
  color: '#FF6D00', lineWidth: 1,
  lastValueVisible: false, priceLineVisible: false,
  crosshairMarkerVisible: false,
}});
ema24Series.setData(data.ema24);
...
// EMA104 白色
const ema104Series = chart1.addLineSeries({{
  color: '#FFFFFF', lineWidth: 1,
  lastValueVisible: false, priceLineVisible: false,
  crosshairMarkerVisible: false,
}});
ema104Series.setData(data.ema104);

const chart2 = LightweightCharts.createChart(document.getElementById('chart2'), {{
  width: w, height: Math.floor(h * 0.28),
  layout: commonLayout, grid: commonGrid, timeScale: commonTimeScale,
  localization: commonLocalization,
  crosshair: {{ mode: LightweightCharts.CrosshairMode.Normal }},
  rightPriceScale: {{ borderColor: '#333' }},
