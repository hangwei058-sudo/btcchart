...
hist = [
    {"time": int(idx.timestamp()), "value": float(row['macd_h']),
     "color": "#26A69A" if row['macd_h'] >= 0 else "#FF5252"}
    for idx, row in df.iterrows() if pd.notna(row['macd_h'])
]
...
seriesMacd = [
    ...
    {"type": 'Line', "data": macd_line, "options": {"color": "blue", "lineWidth": 1}},
    {"type": 'Line', "data": signal_line, "options": {"color": "orange", "lineWidth": 1}},
]
