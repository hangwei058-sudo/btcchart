...
# 🚀 优化：按需动态获取 BTC 数据，避免过量无意义的 1m 请求
@st.cache_data(ttl=120)
def get_btc_base_df(base_type):
    exchange = ccxt.okx({'enableRateLimit': True})
    tf_map = {'minute': ('1m', 1500, 60), 'hour': ('1h', 1500, 3600), 'day': ('1D', 1000, 86400)}
    timeframe, total_bars, sec_per_bar = tf_map.get(base_type, ('1m', 1500, 60))
    
    per_call = 300
    all_bars = []
    now = exchange.milliseconds()
    since = now - (total_bars * sec_per_bar * 1000)
    
    # 仅需 1~5 次请求即可获取足够多的数据
    while since < now and len(all_bars) < total_bars:
        try:
            bars = exchange.fetch_ohlcv('BTC/USDT', timeframe=timeframe, since=since, limit=per_call)
        except Exception:
            break
        if not bars:
            break
    ...
    if not all_bars:
        try:
            all_bars = exchange.fetch_ohlcv('BTC/USDT', timeframe=timeframe, limit=300)
        except Exception:
            all_bars = []
...
# 重采样与指标计算
@st.cache_data(ttl=120)
def resample_data(sym, resample_rule):
    base_type = get_base_interval(resample_rule)
    df = get_gold_df(base_type) if sym == "GC=F" else get_btc_base_df(base_type)
...
# 🚀 优化：使用向量化与 zip 替代低效的 df.iterrows()
candles, ema24_data, ema52_data, ema104_data = [], [], [], []
macd_data, signal_data, hist_data = [], [], []

if not df.empty:
    timestamps = (df.index.astype('int64') // 10**9).tolist()
    ...
    def extract_line_series(series_col, precision=2):
        valid = df[series_col].dropna().round(precision)
        ts = (valid.index.astype('int64') // 10**9).tolist()
        return [{"time": t, "value": v} for t, v in zip(ts, valid.tolist())]

    ema24_data = extract_line_series('ema24', 2)
    ema52_data = extract_line_series('ema52', 2)
    ema104_data = extract_line_series('ema104', 2)
    macd_data = extract_line_series('macd', 4)
    signal_data = extract_line_series('macd_s', 4)

    # 计算 MACD 柱状图颜色
    hist_valid = df['macd_h'].dropna().round(4)
    hist_ts = (hist_valid.index.astype('int64') // 10**9).tolist()
    hist_vals = hist_valid.tolist()

    prev_h = None
    for t, v in zip(hist_ts, hist_vals):
        if v >= 0:
            color = "#26A69A" if (prev_h is None or v >= prev_h) else "#B2DFDB"
        else:
            color = "#FF5252" if (prev_h is None or v <= prev_h) else "#FFCDD2"
