...
with c2:
    st.markdown(
        f"<div style='text-align:center;font-size:20px;font-weight:bold;padding:6px;color:white'>"
...
const chart2 = LightweightCharts.createChart(document.getElementById('chart2'), {{
  width: window.innerWidth,
  height: Math.floor(window.innerHeight * 0.25),
  layout: commonLayout,
  grid: commonGrid,
  timeScale: commonTimeScale,
  crosshair: {{ mode: LightweightCharts.CrosshairMode.Normal }},
...
function syncCrosshair(sourceChart, point) {{
  ...
  charts.forEach(c => {{
    if (c !== sourceChart) {{
      if (point) {{
        c.setCrosshairPosition(point.value, point.time, c.series ? c.series[0] : undefined);
      }} else {{
...
function syncTimeScale(sourceChart, range) {{
  if (isSyncing) return;
  isSyncing = true;
  charts.forEach(c => {{
    if (c !== sourceChart) c.timeScale().setVisibleLogicalRange(range);
  }});
  isSyncing = false;
}}
...
charts.forEach(c => {{
  c.subscribeCrosshairMove(param => {{
    if (param.point) syncCrosshair(c, param);
  }});
  c.timeScale().subscribeVisibleLogicalRangeChange(range => {{
    if (range) syncTimeScale(c, range);
  }});
}});
