...
def fetch_btc_multilevel(rule_str):
    """根据重采样周期动态拉取对应粒度的底层数据，确保大周期具有充足的历史K线"""
    exchange = ccxt.okx({'enableRateLimit': True})
    r = rule_str.lower()
    
    if r.endswith('d'):
        timeframe = '1D'
        target_bars = 1200  # 约 3 年以上日线，合成 15d/45d 足够充裕
        interval_ms = 86400 * 1000
    elif r.endswith('h'):
        timeframe = '1H'
        target_bars = 1500  # 约 60 天小时线
        interval_ms = 3600 * 1000
    else:
        timeframe = '1m'
        target_bars = 4000  # 约 2.7 天 1 分钟线
        interval_ms = 60 * 1000
    ...
    try:
        ...
        since = now - (target_bars * interval_ms)
        while since < now:
        ...
        if not all_bars:
            all_bars = exchange.fetch_ohlcv('BTC/USDT', timeframe=timeframe, limit=per_call)
    except Exception:
        try:
            all_bars = exchange.fetch_ohlcv('BTC/USDT', timeframe='1D', limit=100)
        except Exception:
            all_bars = []
