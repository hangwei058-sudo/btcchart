...
# 默认只聚焦显示最近120根K线，手机上更清晰，可双指缩小查看更早数据
default_view_bars = 700
if len(df) > default_view_bars:
    x_start = df.index[-default_view_bars]
else:
    x_start = df.index[0]
x_end = df.index[-1]
...
fig.update_yaxes(title_text="价格", row=1, col=1, autorange=True)
fig.update_yaxes(title_text="MACD", row=2, col=1, autorange=True)
fig.update_xaxes(rangeslider_visible=False, range=[x_start, x_end])
