...
# 全部压在一行：品种 | 周期 | ◀ | ▶
c1, c2, c3, c4 = st.columns([2, 2, 0.6, 0.6])
...
<style>
  ...
  #info {{ color:#888; font-size:11px; font-family:sans-serif; padding:1px 4px; background:#000; }}
...
const commonGrid   = {{ vertLines:{{color:'#1e1e1e'}}, horzLines:{{color:'#1e1e1e'}} }};
...
// 计算高度：控制栏约48px，info约18px，剩余分配 65% K线 33% MACD
const W  = window.innerWidth;
const TH = window.innerHeight;
const CTRL = 48;
const INFO = 18;
const avail = TH - CTRL - INFO - 4;
const H1 = Math.floor(avail * 0.63);
const H2 = Math.floor(avail * 0.35);
