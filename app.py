...
def resample_data(symbol, rule):
    ...
    # Stoch RSI: K=5, D=3, RSI长度=3, 随机指标长度=14, 来源=收盘价
    rsi_len, stoch_len, k_smooth, d_smooth = 3, 14, 5, 3
    rsi_vals = wilder_rsi(resampled['close'], rsi_len)
    rsi_min = rsi_vals.rolling(stoch_len).min()
    rsi_max = rsi_vals.rolling(stoch_len).max()
    stoch_rsi_raw = (rsi_vals - rsi_min) / (rsi_max - rsi_min) * 100
    resampled['stoch_k'] = stoch_rsi_raw.rolling(k_smooth).mean()
